...
export const HAFITH_INVENTORY: Item[] = [
  ...
  },
  {
    ...
    sku: "KIT-OCK-TOY",
    description:
      "Complete oil change kit for Toyota vehicles with oil, filter, and drain plug",
    is_oil: false,
    isOil: false,
...
export function useHafithInventory(): UseHafithInventoryReturn {
  ...
  useEffect(() => {
    const initializeData = () => {
      try {
        ...
        const storedData = localStorage.getItem(STORAGE_KEYS.HAFITH_ITEMS);
        ...
        if (cachedItems && cachedItems.length > 0) {
          console.log(
            "Loaded Hafith items from localStorage:",
            cachedItems.length,
            "items"
          );
          ...
          localStorage.setItem(
            STORAGE_KEYS.HAFITH_ITEMS,
            JSON.stringify(HAFITH_INVENTORY)
          );
        }
  ...
  useEffect(() => {
    ...
    const updateStorage = () => {
      try {
        localStorage.setItem(STORAGE_KEYS.HAFITH_ITEMS, JSON.stringify(items));
        console.log("Updated Hafith localStorage with", items.length, "items");
      } catch (error) {
        console.error("Error updating Hafith localStorage:", error);
      }
    };
