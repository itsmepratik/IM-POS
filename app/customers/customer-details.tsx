...
import { Phone, Mail, MapPin, Car, Calendar, ClipboardList, FileText } from "lucide-react"
...
export function CustomerDetails({ isOpen, onClose, customer, onEdit }: CustomerDetailsProps) {
  return (
                ...
                <div className="space-y-3 col-span-full">
                  <div className="flex items-center justify-between">
                    ...
                    {customer.lastVisit && (
                      <div className="flex items-center text-sm text-muted-foreground">
                        ...
                        Last Visit: {new Date(customer.lastVisit).toLocaleDateString('en-GB')}
                      </div>
                    )}
                  </div>
                  ...
                  {customer.notes && (
                    <div className="mt-4 space-y-2">
                      ...
                      <p className="text-sm whitespace-pre-wrap pl-6">{customer.notes}</p>
                    </div>
                  )}
                </div>
              </div>
                ...
                {customer.vehicles.length > 0 ? (
                  <Accordion type="multiple" className="w-full">
                    {customer.vehicles.map((vehicle) => (
                      <AccordionItem key={vehicle.id} value={vehicle.id} className="border rounded-md px-4 my-2">
                        ...
                        <AccordionContent className="pb-3 pt-1">
                          <div className="space-y-2">
                            <div className="grid grid-cols-2 gap-2">
                              ...
                              <div>
                                <h4 className="text-xs text-muted-foreground">Year</h4>
                                <p>{vehicle.year || "-"}</p>
                              </div>
                              <div>
                                <h4 className="text-xs text-muted-foreground">License Plate</h4>
                            ...
                            {vehicle.vin && (
                              <div>
                                <h4 className="text-xs text-muted-foreground">VIN</h4>
                            ...
                            {vehicle.notes && (
                              <div>
                                <h4 className="text-xs text-muted-foreground">Notes</h4>
