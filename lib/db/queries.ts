...
export const getCachedProducts = async (locationId: string) => {
    ...
    return unstable_cache(
        async () => {
            ...
            const openBottles = await db.select().from(openBottleDetails).where(eq(openBottleDetails.isEmpty, false));
            ...
            return rawProducts.map(inv => {
                ...
                // Stock Calculation
                const batchStock = inv.batches.length > 0 
                ...
                if (isOilProduct) {
                    ...
                    if (batchStock !== null) {
                         const stockResult = calculateLubricantStock(
                           batchStock,
                           openBottleRows.map((r: any) => ({ current_volume: parseFloat(r.currentVolume) })),
                           volumeInfos
                         );
                         ...
                         if (openBottleRows.length > 0) {
                            totalOpenVolume = openBottleRows.reduce((sum: number, b: any) => sum + (parseFloat(b.currentVolume) || 0), 0);
                         }
                    }
                }

                const finalStock = isOilProduct
                  ? (batchStock !== null ? batchStock : (inv.totalStock || inv.standardStock || 0))
                  : (batchStock !== null ? batchStock : (inv.standardStock || 0));
                ...
                return {
                    ...
                    batteryState: p.batteryState as "new" | "scrap" | "resellable" | null,
