...
export function useCustomers(): UseCustomersReturn {
  ...
  const transformCustomer = useCallback(
    (dbCustomer: any, vehicles: any[] = []): CustomerData => {
    ...
    async (isMounted?: boolean) => {
      ...
      try {
        if (isMounted !== false) {
          ...
          console.log(
            "🔄 useCustomers: Setting loading to true, clearing error"
          );
        }
        ...
        if (customersError) {
          console.error(
            "❌ useCustomers: Supabase query error",
            customersError
          );
        ...
        if (isMounted !== false) {
          console.log(
            "✅ useCustomers: Customers transformed, updating state",
            {
  ...
  const addCustomer = useCallback(
    async (
      ...
      try {
        ...
        if (customerError) {
          console.error(
            "❌ useCustomers: Customer insert error",
            customerError
          );
          throw customerError;
        }
        ...
        if (customerData.vehicles && customerData.vehicles.length > 0) {
          ...
          if (vehiclesError) {
            console.error(
              "❌ useCustomers: Vehicles insert error",
              vehiclesError
            );
            throw vehiclesError;
          }
        ...
        const transformedCustomer = transformCustomer(
          newCustomer,
          vehiclesData
        );

        console.log(
          "📝 useCustomers: Updating customers state with new customer"
        );
        setCustomers((prev) => {
    ...
    async (
      ...
      try {
        ...
        if (updates.email !== undefined)
          customerUpdates.email = updates.email || null;
        if (updates.phone !== undefined)
          customerUpdates.phone = updates.phone || null;
        if (updates.notes !== undefined)
          customerUpdates.notes = updates.notes || null;
        if (updates.lastVisit !== undefined)
          customerUpdates.last_visit = updates.lastVisit || null;
        ...
        if (customerError) {
          console.error(
            "❌ useCustomers: Customer update error",
            customerError
          );
          throw customerError;
        }
        ...
        if (updates.vehicles !== undefined) {
          ...
          if (updates.vehicles.length > 0) {
            ...
            if (vehiclesError) {
              console.error(
                "❌ useCustomers: New vehicles insert error",
                vehiclesError
              );
              throw vehiclesError;
            }
        ...
        console.log(
          "📝 useCustomers: Updating customers state with updated customer"
        );
        setCustomers((prev) => {
  ...
  const deleteCustomer = useCallback(
    async (id: string): Promise<boolean> => {
      console.log("🗑️ useCustomers: Starting deleteCustomer", {
        customerId: id,
      });
  ...
  const getCustomerById = useCallback(
    (id: string): CustomerData | undefined => {
      console.log("🔍 useCustomers: Getting customer by ID", {
        customerId: id,
      });
